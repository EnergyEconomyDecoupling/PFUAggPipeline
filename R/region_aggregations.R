...
#' Aggregate final-to-useful details matrices by regions
#'
#' This is a similar function to `region_pipeline()`,
#' which aggregates the PSUT matrices.
#' This function aggregates the Y_fu and U_EIOU_fu details matrices.
#'
#' The details matrices contain information about the move from
#' final stage to useful stage for both energy and exergy.
#' Details include:
...
#'
#' @param .details_data A data frame of final-to-useful details matrices.
#' @param region_aggregation_map An aggregation map that shows how to
#'                               aggregate countries to regions,
#'                               like FoSUN and FoYUG.
#'                               Entries in the each sublist are not
...
#'         regions, continents, and the World.
#' @export
details_region_pipeline <- function(.details_data,
                                    region_aggregation_map,
                                    continent_aggregation_map,
                                    world_aggregation_map,
                                    country = Recca::psut_cols$country,
  ...
  # So cycle through each Few entry.
  regions <- names(region_aggregation_map)
  Details_Re_regions <- lapply(regions, function(this_region){
    .psut_data |>
      dplyr::left_join(region_aggregation_map[this_region] |>
                         matsbyname::agg_map_to_agg_table(many_colname = country,
                                                          few_colname = region),
                       by = country) |>
      Recca::region_aggregates_details(many_colname = country,
                                       few_colname = region,
                                       drop_na_few = TRUE)
  }) |>
    dplyr::bind_rows()
  ...
  # Create a data frame of continents,
  # by aggregating countries to continents according to continent_aggregation_map.
  Detials_Re_continents <- .details_data |>
    dplyr::left_join(continent_aggregation_map |>
                       matsbyname::agg_map_to_agg_table(many_colname = country,
                                                        few_colname = continent),
                     by = country) |>
  ...
  # Create a data frame of the World,
  # by aggregating continents to World according to world_aggregation_map.
  Details_Re_world <- Details_Re_continents |>
    dplyr::left_join(world_aggregation_map |>
                       matsbyname::agg_map_to_agg_table(many_colname = country,
                                                        few_colname = world),
                     by = country) %>%
  ...
  # Stack the data frames and return
  dplyr::bind_rows(.details_data,
                   Details_Re_regions,
                   Detials_Re_continents,
                   Details_Re_world)
}
